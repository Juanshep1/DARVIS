#!/usr/bin/env python3
"""
S.P.E.C.T.R.A. — Smart Personal Executive for Cognitive Tasks & Real-time Assistance
...
def banner():
  ...
  ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝  ╚═══╝  ╚═╝╚══════╝
    ...
    console.print(
        "  Smart Personal Executive for Cognitive Tasks & Real-time Assistance",
        style=f"bold {GOLD}",
    )
...
def run_startup_actions() -> dict:
    ...
    try:
        ...
        briefing_file = desktop / f"SPECTRA_Briefing_{now.strftime('%Y-%m-%d')}.txt"
        content = f"""
╔══════════════════════════════════════════════════════╗
║           S.P.E.C.T.R.A. DAILY BRIEFING               ║
╚══════════════════════════════════════════════════════╝
...
"""
        ...
        content += f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Generated by S.P.E.C.T.R.A. at {ctx['time']}
"""
...
def _safari_js(js_code: str) -> str:
    ...
    if "Allow JavaScript from Apple Events" in result:
        return ("✗ Safari needs JavaScript enabled for SPECTRA control.\n"
...
def main():
    ...
    try:
        from web import SpectraHandler, ReusableHTTPServer
        ...
        def _run_web():
            try:
                ...
                srv = _web.ReusableHTTPServer(('0.0.0.0', WEB_PORT), _web.SpectraHandler)
                srv.serve_forever()
            except Exception:
                pass
    ...
    console.print(
        Panel(
            ...
            + "[bold]goodbye[/bold]     — exit S.P.E.C.T.R.A.",
            title=f"[bold {CYAN}]S.P.E.C.T.R.A. Ready[/bold {CYAN}]",
            border_style=CYAN,
        )
    )
...
3. Create a file on the Desktop called SPECTRA_Morning_Briefing.txt with the date, weather, and top 5 news stories with summaries
...
3. Create a file on the Desktop called SPECTRA_Evening_Briefing.txt with the date, weather, and top 5 news stories with summaries
4. Open that file with open_file
5. Navigate Safari to https://news.google.com using safari navigate
6. After all commands execute, give a spoken evening wrap-up: what happened today in the news, tomorrow's weather, reminders, wish good night.""",
    ...
    from scheduler import SPECTRAScheduler, _global_scheduler
    import scheduler as _sched_module
    spectra_scheduler = SPECTRAScheduler()
    _sched_module._global_scheduler = spectra_scheduler
    spectra_scheduler.sync_from_cloud()
    if spectra_scheduler.tasks:
        console.print(f"  [green]✓[/green] {len(spectra_scheduler.tasks)} scheduled task(s) loaded")
    ...
    def _scheduler_loop():
        ...
        while True:
            _time.sleep(5)
            try:
                spectra_scheduler.check_and_run(brain, extract_and_run_commands, console, tts)
                spectra_scheduler.sync_from_cloud()  # Pick up tasks from other devices
            except Exception:
                pass
    ...
    def _background_listener():
        ...
        while not _listen_stop.is_set():
            ...
            if result and listening_active:
                if ambient_mode:
                    if result and "spectra" in result.lower():
                        cleaned = re.sub(r'(?i)\b(hey\s+)?spectra\b', '', result).strip()
...
5. Create a DETAILED briefing file on the Desktop called SPECTRA_Briefing_{now.strftime('%Y-%m-%d')}.txt with:
...
8. Use safari navigate to open https://wttr.in/Springfield in a new Safari tab""")
                        ...
                        if summary:
                            console.print(Panel(Markdown(summary), title=f"[bold {CYAN}]S.P.E.C.T.R.A. Briefing[/bold {CYAN}]", border_style=BLUE, padding=(1, 2)))
                            tts.speak(summary)
                            tts.wait_for_speech()
                    except Exception as e:
            ...
            if lower == "/ambient":
                ...
                if has_mic:
                    listening_active = True
                console.print(f"  [green]✓[/green] Ambient mode — say [bold]'SPECTRA'[/bold] to wake. /active to exit.")
                continue
            ...
            if any(t in lower for t in screen_triggers):
                ...
                display = re.sub(r'```command\s*\n.*?\n```', '', response, flags=re.DOTALL).strip()
                if display:
                    console.print(Panel(Markdown(display), title=f"[bold {CYAN}]S.P.E.C.T.R.A.[/bold {CYAN}]", border_style=BLUE))
                    tts.speak(display[:300])
                    tts.wait_for_speech()
                continue
                    ...
                    if display_text:
                        ...
                        console.print(
                            Panel(
                                ...
                                title=f"[bold {CYAN}]S.P.E.C.T.R.A. (Gemini)[/bold {CYAN}]",
                                border_style=BLUE,
                                padding=(1, 2),
                            )
                        )
            ...
            if display_text:
                ...
                console.print(
                    Panel(
                        Markdown(display_text),
                        title=f"[bold {CYAN}]S.P.E.C.T.R.A.[/bold {CYAN}]",
                        border_style=BLUE,
                        padding=(1, 2),
                    )
                )
